...
Ocypod uses [Redis](https://redis.io/) as a storage backend, and it's recommended to use a dedicated Redis instance (or at least its own DB) to avoid any clashes with other key spaces.

The latest [stable release](https://redis.io/download) (5.0) is recommended, though there are no known issues using 4.0 or 3.2.
...
Create a new queue named "demo" with a default job timeout of 10 minutes, and everything else as default:
...
Then verify that the newly created queue exists, and see its full configuration:
...
### Start adding jobs to the queue

Next, add a job to this newly created queue:

    $ curl -i -H 'content-type: application/json' -XPOST -d '{"foo": "bar", "items": [1,2,3]}' localhost:8023/queue/example/job
    HTTP/1.1 201 Created
    content-length: 2
    location: /job/1
    date: Fri, 16 Nov 2018 17:57:22 GMT

    1

This creates a new job in the queue, with an auto-generated job ID (1 in this case).

You can check the job's status at the job's URL:

    $ curl localhost:8023/job/1/status
    "queued"

Or you can get the all job-related fields from:
	...
	{"id":60,
     ...
     "status":"queued",
     "tags":null,
	 "created_at":"2018-11-16T17:57:22.075505Z",
     "started_at":null,
     "ended_at":null,
     "last_heartbeat":null,
     "input": {"foo":"bar","items":[1,2,3]},
     "output": null,
     "timeout": "10m",
...
`input` is the job data that workers will get to work on. They'll then write their results, progress, error information etc. to the `output` field as desired.

### Start and complete a job

We can simulate the role of a worker, and ask for a job, update some job data, then mark the job as completed.
...
Ask the `demo` queue for a job:
	...
	date: Tue, 20 Nov 2018 09:42:45 GMT

	{"id":60,"input":null}

Mark the job as complete and write some results:

    $ curl -i -XPATCH -H 'content-type: application/json' -d '{"status": "completed", "output": {"my_result": 1234, "files_processed": 10, "ids_skipped": [4, 10, 11]}}' localhost:8023/job/61
    HTTP/1.1 204 No Content
    date: Tue, 20 Nov 2018 09:49:13 GMT

If all went well, the job will have been marked as completed, its `output` field updated, and a 204 response code returned.

You can verify this by checking the job's metadata:

    $ curl -i 'http://localhost:8023/job/61?fields=status,ended_at,output'
    {"status": "completed",
     "ended_at": "2018-11-20T09:49:13.589720Z",
     "output": {"files_processed":10,"ids_skipped":[4,10,11],"my_result":1234}}
...
TODO: links to documentation for fields.
...
TODO: creating a new queue
TODO: creating a new job
TODO: getting a new job
TODO: updating job heartbeat
TODO: updating job progress
TODO: completing a job

## High level concepts

### Job

A job in Ocypod represents some task created by clients, and to be completed by workers.
...
When a job is initially created, it's added to a queue, assigned the `created` status.
...
Ocypod supports any number of queues, each with their default job settings.

A queue in Ocypod is a [FIFO](https://en.wikipedia.org/wiki/FIFO_(computing_and_electronics)), with new jobs being added to the beginning of the queue, and workers taking jobs from the end of the queue.
...
One of the goals of Ocypod was to make writing clients/workers a fairly straightforward process, since all communication takes place using HTTP. It's also designed to be fairly flexible, to allow for different patterns/workflows.

The simplest possible worker would generally do the following in a loop:

1. poll a queue (or queues) for a job to work on
2. when it gets a job, start processing the job
3. when job is complete, update the job's status to `completed`

A more fully complex client might do the following:
